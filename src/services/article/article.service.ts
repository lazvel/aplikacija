...
export class ArticleService extends TypeOrmCrudService<Article> {
    ...
    async editFullArticle(articleId: number, data: EditArticleDto): Promise<Article | ApiResponse> {
        ...
        const newPrice: string = Number(data.price).toFixed(2); // 50 -> 50.00
    ...
    async search(data: ArticleSearchDto): Promise<Article[] | ApiResponse> {
        ...
        // pametnije resenje bi bilo "ap.current = 1" (zahteva trigger_article_price_ai)
        ...
        if (data.priceMax && typeof data.priceMin === 'number') {
        ...
        if (articleIds.length === 0) {
            return new ApiResponse("ok", 0, "No results found for these search parameters.");
        }
