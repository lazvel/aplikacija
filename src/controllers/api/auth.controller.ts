...
import { Controller, Post, Body, Req, Put, HttpException, HttpStatus, UseGuards } from "@nestjs/common";
...
export class AuthController {
    constructor(
        public adminstratorService: AdministratorService, 
    ...
    @Post('user/register') // auth/user/register
    ...
    async doUserLogin(@Body() data: LoginUserDto, @Req() req: Request): Promise<LoginInfoDto | ApiResponse> {
        ...
        // ovo je malo komplikovanije moramo da izmenimo u posebnoj fji za database format niji isti kao iso
    ...
    async userTokenRefresh(@Req() req: Request, @Body() data: UserRefreshTokenDto): Promise<LoginInfoDto | ApiResponse> {
        ...
        if (!userToken) {
            return new ApiResponse("error", -10002, "No such token");
        }
